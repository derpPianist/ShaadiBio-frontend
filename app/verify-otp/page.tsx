...
export default function VerifyOtpPage() {
  ...
  const handleOtpSubmit = async () => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/v1/auth/validate", {
        method: "POST",
        credentials: "include",
        headers: {
      ...
      console.log("Data after successful verification of OTP: ", data);

      setAccessToken(data.accessToken);

      router.push(
        `/dashboard?full_name=${encodeURIComponent(data.account.full_name)}`,
      );
    } catch (error) {
      console.log("Error while calling /validate api: ", error);
    }
  };
    ...
    >
      ...
      >
        ...
        >
          ...
          <Typography
            variant="body2"
            sx={{ color: isDark ? "#aaa" : "#666", mb: 5, lineHeight: 1.6 }}
          >
          ...
          <Box
            sx={{ display: "flex", gap: 1.5, justifyContent: "center", mb: 5 }}
          >
          ...
          <Typography
            variant="body2"
            sx={{ color: isDark ? "#ccc" : "#555", textAlign: "center", mb: 4 }}
          >
            Didn&apos;t get the code?{" "}
            <MuiLink
              href="#"
              underline="always"
              sx={{ color: isDark ? "#fff" : "#1a1a1a", fontWeight: 600 }}
            >
              Resend code
            </MuiLink>
          </Typography>
          ...
          >
            <MuiLink
              href="#"
              underline="hover"
              sx={{ color: isDark ? "#888" : "#888", fontSize: "0.875rem" }}
            >
              Need help?
            </MuiLink>
            <MuiLink
